...
func (ns *linuxNs) Init(container *libcontainer.Container, uncleanRootfs, console string, syncPipe *SyncPipe, args []string) error {
	...
	if err := label.SetProcessLabel(container.Context["ProcessLabel"]); err != nil {
		return fmt.Errorf("SetProcessLabel label %s", err)
	}
	if err := setupNewMountNamespace(rootfs, container.Mounts, console, container.ReadonlyFs, container.NoPivotRoot, container.Context["MountLabel"]); err != nil {
