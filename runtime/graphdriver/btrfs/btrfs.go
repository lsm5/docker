...
func subvolCreate(path, name string, label string) error {
...
func (d *Driver) Create(id string, parent string, label string) error {
	subvolumes := path.Join(d.home, "subvolumes")
	if err := os.MkdirAll(subvolumes, 0700); err != nil {
		return err
	}
	if parent == "" {
		if err := subvolCreate(subvolumes, id, label); err != nil {
			return err
		}
	} else {
